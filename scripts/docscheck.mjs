...
async function files(dir) {
  ...
  for (const entry of list) {
    ...
    if (entry.isFile() && path.endsWith(".mdx")) {
      output.push(path);
    }
  }
  return output;
...
function doclinks(text) {
  return [...text.matchAll(/\[[^\]]+\]\(([^)\s]+)(?:\s+"[^"]*")?\)/g)]
    ...
    .filter(
      (target) =>
        target && !target.startsWith("#") && !target.startsWith("http")
    );
}
...
function route(file) {
  ...
  if (relative.endsWith("/index.mdx")) {
    return `/docs/${relative.slice(0, -"/index.mdx".length)}`;
  }
...
for (const file of list) {
  ...
  if (path !== "/docs") {
    for (const part of path.slice("/docs/".length).split("/")) {
      if (!segment.test(part)) {
        issues.push(`${name}: invalid route segment ${part}`);
      }
    }
  }
  let parsed;
  ...
  for (const target of anchorlinks(clean)) {
    if (!seen.has(target)) {
      issues.push(`${name}: missing anchor target #${target}`);
    }
  }
  for (const target of doclinks(clean)) {
    if (target.startsWith("/docs")) {
      ...
      if (!routes.has(pathname)) {
        issues.push(`${name}: missing docs route ${pathname}`);
      }
      if (anchor) {
        ...
        if (linked) {
          ...
          if (!ids.has(anchor)) {
            issues.push(`${name}: missing docs anchor ${pathname}#${anchor}`);
          }
        }
      }
      continue;
    ...
    if (target.startsWith(".")) {
      ...
      if (!resolved) {
        issues.push(`${name}: missing local docs file ${target}`);
      }
      const { anchor } = parsepath(target);
      if (resolved && anchor) {
        const ids = await anchorsfor(resolved);
        if (!ids.has(anchor)) {
          issues.push(`${name}: missing local docs anchor ${target}`);
        }
      }
    }
  }

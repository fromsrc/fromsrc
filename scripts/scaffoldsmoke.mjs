...
try {
  for (const framework of frameworks) {
    ...
    const create = await run(
      "node",
      [bin, "--name", name, "--framework", framework, "--yes"],
      work
    );
    if (create.code !== 0) {
    ...
    if (install.code !== 0) {
      ...
      if (install.out.trim()) {
        console.error(install.out.trim());
      }
      if (install.err.trim()) {
        console.error(install.err.trim());
      }
      continue;
    }
    ...
    if (typecheck.code !== 0) {
      ...
      if (typecheck.out.trim()) {
        console.error(typecheck.out.trim());
      }
      if (typecheck.err.trim()) {
        console.error(typecheck.err.trim());
      }
      continue;
    }
    ...
    if (build.code !== 0) {
      ...
      if (build.out.trim()) {
        console.error(build.out.trim());
      }
      if (build.err.trim()) {
        console.error(build.err.trim());
      }
      continue;
    }
...
console.log(
  `o scaffold smoke validation passed (${frameworks.length} frameworks)`
);

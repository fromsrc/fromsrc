...
if (adapterpaths.length !== frameworks.length) {
  issues.push(
    `adapterpaths count mismatch ${adapterpaths.length}/${frameworks.length}`
  );
}
...
if (adapters.length !== frameworks.length) {
  issues.push(
    `adapters count mismatch ${adapters.length}/${frameworks.length}`
  );
}
...
for (const manual of manuals) {
  if (
    !manual.file.startsWith("docs/manual/") ||
    !manual.file.endsWith(".mdx")
  ) {
...
for (const path of adapterpaths) {
  ...
  if (owners.length !== 1) {
    issues.push(
      `adapter path ${path} must appear in exactly one manual token list`
    );
  }
}
...
console.log(
  `o frameworkset contract validation passed (${frameworks.length} frameworks)`
);

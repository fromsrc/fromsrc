...
function parseEnum(text) {
  ...
  return unique(
    [...block[1].matchAll(/"([^"]+)"/g)].map((item) => item[1] ?? "")
  );
}
...
for (const name of needed) {
  const mark = `- \`${name}\``;
  if (!docs.includes(mark)) {
    issues.push(`docs method missing: ${name}`);
  }
}

if (!manifest.includes("resources:")) {
  issues.push("manifest resources capability missing");
}
if (!manifest.includes("listChanged: false")) {
  issues.push("manifest listChanged missing");
}
if (!ops.includes("fromsrc://docs/{slug}")) {
  issues.push("resource template missing");
}
...
if (!rpc.includes("export const page = z.object({ slug })")) {
  issues.push("page schema not bound to slug schema");
}

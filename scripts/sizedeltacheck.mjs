...
try {
  for (const item of list) {
    ...
    const limit =
      mark.size < 2000
        ? Math.max(Math.ceil(mark.size * 0.08), 120)
        : Math.max(Math.ceil(mark.size * 0.08), 700);
    const limitgzip =
      mark.gzip < 1000
        ? Math.max(Math.ceil(mark.gzip * 0.08), 80)
        : Math.max(Math.ceil(mark.gzip * 0.08), 300);
    if (grow > limit || growgzip > limitgzip) {
      ...
      console.log(
        `o ${item.name}: +${grow}b/+${growgzip}b within +${limit}b/+${limitgzip}b`
      );
    }
  }
} finally {

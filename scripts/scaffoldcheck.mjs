...
const frameworks = [
  ...
  },
  {
    checks: [
      ...
      {
        file: "package.json",
        text: '"start": "remix-serve ./build/server/index.js"',
      },
      { file: "package.json", text: '"typecheck": "tsc --noEmit"' },
      { file: "tsconfig.json", text: '"jsx": "react-jsx"' },
      { file: "package.json", text: '"node": "^20.19.0 || >=22.12.0"' },
...
const missingframeworks = sharedframeworks.filter(
  (name) => !localframeworks.includes(name)
);
const extraframeworks = localframeworks.filter(
...
if (
  ...
  localframeworks.length !== sharedframeworks.length
) {
  ...
  if (localframeworks.length !== sharedframeworks.length) {
    issues.push(
      `framework list count mismatch ${localframeworks.length}/${sharedframeworks.length}`
    );
  }
}
...
try {
  ...
  for (const framework of frameworks) {
    ...
    generate({ framework: framework.name, name });
...
console.log(
  `o scaffold contract validation passed (${frameworks.length} frameworks)`
);

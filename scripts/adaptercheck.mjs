...
for (const item of adapters) {
  ...
  if (!text.includes(`"${"use client"}"`)) {
    issues.push(`${item.file} missing use client`);
  }
  if (!text.includes(`export { ${item.name} }`)) {
    issues.push(`${item.file} missing ${item.name} export`);
  }
  for (const name of shared) {
    if (!text.includes(name)) {
      issues.push(`${item.file} missing ${name} export`);
    }
  }
}
...
if (doc) {
  for (const item of adapters) {
    if (!doc.includes(`\`${item.path}\``)) {
      issues.push(`framework docs missing ${item.path}`);
    }
  }
}
...
console.log(
  `o adapter contract validation passed (${adapters.length} adapters)`
);

...
const issues = [];
if (!build) {
  issues.push("missing scripts.build");
}
if (!build.includes("tsup")) {
  issues.push("scripts.build must run tsup");
}
if (/\brm\s+-rf\s+dist\b/.test(build)) {
  issues.push("scripts.build must not delete dist before build");
}

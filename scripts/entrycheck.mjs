...
for (const entry of entryfiles) {
  ...
  for (const node of source.statements) {
    if (ts.isExpressionStatement(node) && !isdirective(node)) {
      issues.push(
        `${path.relative(root, file)}: top-level side effect statement is not allowed`
      );
      continue;
    }
    if (ts.isExportAssignment(node)) {
      issues.push(
        `${path.relative(root, file)}: default export is not allowed`
      );
      continue;
    }
    if (ts.isExportDeclaration(node)) {
      ...
      issues.push(
        `${path.relative(root, file)}: wildcard re-export is not allowed`
      );
    }
  }
}

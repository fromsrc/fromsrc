...
import {
  defineContent,
  getNavigation as getnav,
} from "../packages/fromsrc/src/content.ts";
...
const [local, exported] = await Promise.all([
...
if (!localkeys.includes("manual")) {
  issues.push("defineContent navigation missing manual section");
}
if (!exportedkeys.includes("manual")) {
  issues.push("exported navigation missing manual section");
}
if (localkeys.join("|") !== exportedkeys.join("|")) {
  issues.push("navigation section order mismatch");
}
...
for (const section of local) {
  ...
  const right =
    exported.find((item) => key(item) === sectionKey)?.items.length ?? 0;
  if (left !== right) {
    issues.push(
      `navigation item count mismatch for ${sectionKey || "root"}: ${left} vs ${right}`
    );
  }
}

const manualmeta = await loadMeta(join(docsdir, "manual"));
const manualpages = (manualmeta?.pages ?? []).filter(
  (item) => item !== "index"
);
const expected = [
  "manual/index",
  ...manualpages.map((item) => `manual/${item}`),
];
const actual =
  local
    .find((item) => key(item) === "manual")
...
console.log(
  `o navigation contract validation passed (${localtitles.length} sections)`
);

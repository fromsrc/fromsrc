...
for (const [key, source] of entrymap.entries()) {
  ...
  if (typeof value === "string") {
    if (!key.endsWith(".css")) {
      issues.push(`${key} must use types/import export object`);
    }
    continue;
  }
  ...
  if (!types.endsWith(`/${base}.d.ts`)) {
    issues.push(`${key} has unexpected types path ${types}`);
  }
  if (!runtime.endsWith(`/${base}.js`)) {
    issues.push(`${key} has unexpected import path ${runtime}`);
  }
  if (!source) {
    continue;
  }
...
for (const key of Object.keys(map)) {
  if (!entrymap.has(key)) {
    issues.push(`unexpected export key ${key}`);
  }
}

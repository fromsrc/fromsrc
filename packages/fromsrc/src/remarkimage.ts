...
function parseDimensions(alt: string): {
  alt: string;
  width?: number;
  height?: number;
} {
  ...
  return { alt: text, height: Number(height), width: Number(width) };
}
...
function createImageProperties(
  ...
  if (parsed.width) {
    props.width = parsed.width;
  }
  if (parsed.height) {
    props.height = parsed.height;
  }
  return props;
}
...
export const remarkImage: Plugin<[RemarkImageOptions?], Root> = (
  options = {}
) => {
        ...
        (match: string, attrs: string) => {
          ...
          if (basePath) {
            result = result.replaceAll(
              /src="(?!https?:\/\/|\/\/)([^"]+)"/g,
              (_, src) => `src="${resolveUrl(src, basePath)}"`
            );
          }
          return result;
        }

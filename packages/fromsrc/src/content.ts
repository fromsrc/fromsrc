...
import { loadMeta, sortByMeta } from "./meta";
...
export function defineContent<T extends SchemaType>(config: ContentConfig<T>) {
  ...
  let searchCache: (Meta & { content: string; headings?: Heading[] })[] | null =
    null;

  async function getSearchDocs(): Promise<
    (Meta & { content: string; headings?: Heading[] })[]
  > {
    if (isProduction() && searchCache) {
      return searchCache;
    }
    if (isProduction() && fullDocsCache) {

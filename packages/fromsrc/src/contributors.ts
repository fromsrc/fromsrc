...
export async function getContributors(
  ...
  try {
    const { stdout } = await run(
      "git",
      ["log", "--format=%aN|%aE|%aI", "--", config.file],
      {
        cwd: config.dir,
      }
    );
    ...
    for (const line of stdout.trim().split("\n")) {
      ...
      if (existing) {
        existing.commits++;
        if (date > existing.lastCommit) {
          existing.lastCommit = date;
        }
      } else {
...
export async function getEditHistory(
  ...
  try {
    const { stdout } = await run(
      "git",
      ["log", "--format=%H|%aN|%aI|%s", "--", config.file],
      {
        cwd: config.dir,
      }
    );
    ...
    return stdout
      ...
      .map((line) => {
        ...
        return { author, date, hash, message: parts.slice(3).join("|") };
      })
      .filter(
        (
          item

...
function text(node: ReactNode): string {
  ...
  if (Array.isArray(node)) {
    return node.map((item) => text(item)).join(" ");
  }
...
const components = {
  ...
  h1: (props: HeadingProps): JSX.Element => (
    <h1
      className="text-2xl font-medium mt-12 mb-4 first:mt-0 text-fg"
      {...props}
    />
  ),
  ...
  hr: (): JSX.Element => (
    <hr className="border-line my-12" aria-hidden="true" />
  ),
  ...
  ol: (props: ListProps): JSX.Element => (
    <ol
      className="my-4 space-y-2 text-muted list-decimal list-inside"
      role="list"
      {...props}
    />
  ),
  ...
  thead: (props: TheadProps): JSX.Element => (

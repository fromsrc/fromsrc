...
export function Dropdown({
  ...
  const selectableItems = items.filter(
    (i): i is DropdownItem => i !== "separator" && !i.disabled
  );
  ...
  const getItemId = useCallback(
    (itemIndex: number): string => `${id}-item-${itemIndex}`,
    [id]
  );
  ...
  const handleItemHover = useCallback(
    (item: DropdownItem, selectableIndex: number): void => {
      if (!item.disabled) {
    ...
    <div ref={ref} className="relative inline-block">
      <button
        ...
        aria-activedescendant={
          open && index >= 0 ? getItemId(index) : undefined
        }
      >
        {trigger}
      </button>

...
function PlaygroundBase({
  ...
  const removeHeader = useCallback(
    (i: number) => setHeaders((h) => h.filter((_, idx) => idx !== i)),
    []
  );
  ...
  const input =
    "w-full rounded border border-line bg-transparent px-2 py-1 text-sm";
  ...
  return (
    ...
    >
      ...
      <div className="p-4 space-y-4">
        ...
        {hasBody && (
          <div>
            <span className="text-xs font-medium text-muted uppercase mb-2 block">
              Body
            </span>
            <textarea

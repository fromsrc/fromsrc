...
const allLanguages: CodeLanguage[] = [
  "javascript",
  "python",
  "curl",
  "go",
...
function javascript(c: CodeSampleConfig): string {
  ...
  if (c.body) {
    opts.push(`\tbody: JSON.stringify(${JSON.stringify(c.body)})`);
  }
...
function python(c: CodeSampleConfig): string {
  ...
  if (c.body) {
    lines.push(`data = ${JSON.stringify(c.body)}`);
    args.push("json=data");
  }
  lines.push(
    `response = requests.${c.method.toLowerCase()}(${args.join(", ")})`
  );
  return lines.join("\n");
}
...
function go(c: CodeSampleConfig): string {
  const h = merged(c);
  const body = c.body
    ? `bytes.NewBuffer([]byte(\`${JSON.stringify(c.body)}\`))`
    : "nil";
  const lines = [
...
function ruby(c: CodeSampleConfig): string {
  ...
  if (c.body) {
    lines.push(`request.body = ${JSON.stringify(c.body)}.to_json`);
  }
...
function php(c: CodeSampleConfig): string {
  ...
  if (c.body) {
    opts.push(`\t"content" => json_encode(${JSON.stringify(c.body)})`);
  }
  return [
...
export function generateSample(
  ...
  return {
    code: generators[language](config),

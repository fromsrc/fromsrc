...
function backtrack(
  ...
  while (i > 0 || j > 0) {
    ...
    const bj = ignore ? bjRaw.trim() : bjRaw;
    if (i > 0 && j > 0 && ai === bj) {
      result.push({ content: aiRaw, newLine: j, oldLine: i, type: "same" });
      i--;
      j--;
    } else if (
      j > 0 &&
      ...
      result.push({ content: bjRaw, newLine: j, type: "add" });
      j--;
    } else {
      result.push({ content: aiRaw, oldLine: i, type: "remove" });
      i--;
    }
  }
...
export function diffLines(
  ...
  return { additions, lines, removals, unchanged };
}
...
export function diffFrontmatter(
  ...
  for (const key of keys) {
    if (oldFields[key] !== newFields[key]) {
      result[key] = { new: newFields[key], old: oldFields[key] };
    }
  }
  return result;
}

...
export function createGithubSource(config: GithubSourceConfig): ContentSource {
  ...
  async function tree(
    ...
    if (!res.ok) {
      ...
      return cached
        ? { entries: cached.entries, truncated: cached.truncated }
    ...
    const entries = (json.tree ?? [])
      .filter(
        (item): item is { path: string; type: "blob" | "tree"; sha: string } =>
    ...
    try {
      ...
      const docs: DocMeta[] = entries
        ...
        .map((item) => {
          ...
          const fallbackTitle =
            slug === "" ? "index" : (slug.split("/").pop() ?? slug);
  ...
  const get = async (
    ...
    const candidates = knownPath
      ? [knownPath, filepath, indexPath]
      : [filepath, indexPath];

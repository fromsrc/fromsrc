...
type InferSchema<T extends CollectionConfig<z.ZodRawShape>> = z.infer<
  T["schema"]
>;
  ...
  async function walk(current: string, prefix = ""): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        if (
          !includeDrafts &&
          (data as Record<string, unknown>).draft === true
        ) {
          continue;
        }
        ...
        items.push({ content, data: parsed.data, slug });
      }
    }
  }
...
export function defineCollection<T extends z.ZodRawShape>(
  ...
  async function load(): Promise<Item[]> {
    ...
    const includeDrafts =
      config.includeDrafts ?? process.env.NODE_ENV !== "production";
...
export function defineCollections<
  const T extends Record<string, CollectionConfig<z.ZodRawShape>>,
>(configs: T): { [K in keyof T]: Collection<z.infer<T[K]["schema"]>> } {

...
export function resolveSchema(
  ...
  if (Array.isArray(raw.oneOf)) {
    schema.oneOf = raw.oneOf.map((schemaitem) =>
      resolveSchema(root, schemaitem, depth + 1, refs)
    );
  }
  if (Array.isArray(raw.anyOf)) {
    schema.anyOf = raw.anyOf.map((schemaitem) =>
      resolveSchema(root, schemaitem, depth + 1, refs)
    );
  }
  if (Array.isArray(raw.allOf)) {
    schema.allOf = raw.allOf.map((schemaitem) =>
      resolveSchema(root, schemaitem, depth + 1, refs)
    );
  }
